...
class Minor_node_centred_Subgraph:
    def __init__(self, graph, dataframe,k_rw,k_khop,k_ego):
        ...
        self.k_khop = k_khop
        self.k_ego = k_ego
    def get_fraud_subgraph(self, k_rw=10, k_khop=2, k_ego=3):
        fraud_nodes = np.where(np.array(self.dataframe['Class']) == 1)[0]

        # # Random walk nodes
        rw_nodes = dgl.sampling.random_walk(self.graph, fraud_nodes, length=k_rw)[0]
        ...
        # k-hop subgraph
        sg, inverse_indices = dgl.khop_in_subgraph(self.graph, fraud_nodes, k=k_khop)
        ...
        # k-ego subgraph
        ego_subgraph, inverse_indices = dgl.khop_in_subgraph(self.graph, all_neighbors, k=k_ego)
...
if __name__ == "__main__":
    ...
    g = dgl.batch(graphs)
    dgl.save_graphs("ccfraud_graph.bin", [g])
    print(g)
