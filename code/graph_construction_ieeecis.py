...
if __name__ == "__main__":
    ...
    # 读取数据集
    df = pd.read_csv(r"C:\Users\59704\Desktop\GITHUB\ieeecis.csv")
    print("Data loaded.")
    ...
    # 构建图
    g = dgl.graph((edges[0], edges[1]), num_nodes=len(df['TransactionID'].unique()))
    # 随机生成train_mask、val_mask、test_mask
    n = len(df['TransactionID'].unique())
    ...
    g.ndata['labels'] = torch.LongTensor(np.array(df['isFraud']))
