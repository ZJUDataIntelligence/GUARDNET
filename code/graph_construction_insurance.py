...
if __name__ == "__main__":
    ...
    # 读取数据集
    df_temp = pd.read_csv(r"insurance.csv")
    print("Data loaded.")
    features = np.array(df_temp.drop("FraudFound_P",axis = 1))
    ...
    g.ndata['labels'] = torch.LongTensor(np.array(df_temp["FraudFound_P"]))



    # 使用例子
    # graph 和 df 是你的图和数据框
    processor = Minor_node_centred_Subgraph(g, df_temp,args.k_rw,args.k_khop,args.k_ego)
