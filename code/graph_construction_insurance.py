...
class Minor_node_centred_Subgraph:
    ...
    def get_fraud_subgraph(self):
        fraud_nodes = np.where(np.array(self.dataframe.iloc[:, -1]) == 1)[0]
...
def get_args():
    ...
    parser.add_argument('--gamma','-gamma',type=float,default=0.6)
...
if __name__ == "__main__":
    ...
    # 如果需要排除 FraudFound_P，确保它不在 features_columns 中
    if 'FraudFound_P' in features_columns:
        features_columns.remove('FraudFound_P')
    ...
    g.ndata['labels'] = torch.LongTensor(np.array(df.iloc[:, -1]))
    ...
    sampler = Major_node_centred_downsampling(g, args.gamma)
    sampled_graph = sampler.apply_mlp()
    graphs = [sampled_graph, result_graph]
    g = dgl.batch(graphs)
    dgl.save_graphs("insurance_graph.bin", [g])
    print(g)
