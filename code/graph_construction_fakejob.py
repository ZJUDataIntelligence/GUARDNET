...
class Minor_node_centred_Subgraph:
    ...
    def get_fraud_subgraph(self):
        fraud_nodes = np.where(np.array(self.dataframe['fraudulent']) == 1)[0]
...
def get_args():
    ...
    parser.add_argument('--input_size','-input_size',type=int,default=308)
...
if __name__ == "__main__":
    ...
    # 读取数据集
    main_data = pd.read_csv(r"fakejob.csv")
    print("Data loaded.")
    features = np.array(main_data.iloc[:, :-1])
    ...
    # 构建图
    g = dgl.graph((edges[0], edges[1]), num_nodes=len(main_data))
    ...
    g.ndata['labels'] = torch.LongTensor(np.array(main_data.iloc[:, -1]))


    # 使用例子
    # graph 和 df 是你的图和数据框
    processor = Minor_node_centred_Subgraph(g, main_data,args.k_rw,args.k_khop,args.k_ego)
    ...
    g = dgl.batch(graphs)
    dgl.save_graphs("fakejob_graph.bin", [g])
    print(g)
